...
var previous_rat_position = Vector3(0,0,0)
...
func _process(delta: float) -> void:
	...
	if rat != null:
		if time_since_last_ripple >= TIME_BETWEEN_RIPPLES:
			if rat.position.distance_squared_to(previous_rat_position) > 0.0:
				create_ripple(rat.position)
				
				time_since_last_ripple = 0
		time_since_last_ripple += delta
		previous_rat_position = rat.position
